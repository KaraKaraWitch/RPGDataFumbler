        ...
        if cls:
            if (tl_folder / rel).exists():
                ...
                if maps:
                    ...
                    (folders["export"] / rel).with_suffix(".nt.txt").write_text(
                        maps, encoding="utf-8"
                    )
...
def create_maps(folders: typing.Dict[str, pathlib.Path], config_dict):
    ...
    for script_file in folders["scripts"].rglob("*.js"):
        ...
        (tl_folder / rel).parent.mkdir(parents=True, exist_ok=True)
        if (
            not (tl_folder / rel).exists()
            and not config_dict["General"]["replace_scripts"]
        ):
            logger.info(f"Dumping: {rel.name}")
...
def patch(
    game_exec: pathlib.Path, folders: typing.Dict[str, pathlib.Path], config_dict
):
    for file in game_exec.iterdir():
        if file.is_dir() and (
            file.name == project_folder_name or (file / ".TLPROJECT").exists()
        ):
